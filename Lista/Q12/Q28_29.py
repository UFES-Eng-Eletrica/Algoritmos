...
porem agora com outra lei aplicada no circuito.
...
def F(x, y):
    F = np.zeros(1)
    F[0] = -1.5*y[0] + 1.5*y[0]**3
    return F

#utilizando o metodo de euler
Xplot, Yplot = integrate(F, 0, np.array([0.5]), 1, 10)
ax1 = plt.plot(Xplot,Yplot, '-')



#utilizando o metodo de RK4
X2plot, Y2plot = integrateRK4(F,0,np.array([0.5]),0.1,10)
